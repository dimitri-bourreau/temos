...
export function FloatingTimerBar() {
  ...
  const handleStart = useCallback(
    async (newCategoryId: string, newTaskId: string) => {
      await start(newCategoryId, newTaskId);
    },
    [start]
  );
  ...
  return (
    <>
      <div className="shrink-0 border-t border-border bg-background/95 shadow-lg backdrop-blur-sm mb-13 md:mb-0">
        {isRunning ? (
          ...
          <div className="flex items-center px-4 py-3">
            <button
              onClick={() => setTaskPickerOpen(true)}
              className="flex flex-1 items-center gap-2 rounded px-3 py-1.5 text-sm text-muted-foreground hover:bg-muted transition-colors"
            >
      ...
      <TimerTaskPickerDialog
        ...
        onSelect={isRunning ? changeTask : handleStart}
      />
